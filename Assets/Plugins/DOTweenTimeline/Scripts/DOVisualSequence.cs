...
#pragma warning restore 0649
        ...
        /// <summary>
        /// INTERNAL USE ONLY: Parameterless constructor that uses the GUID already assigned (for sequences not created via <see cref="DOVisualSequencer"/>)
        /// </summary>
...
//            Debug.Log("<color=#ff0000>AFTER DESERIALIZE ► Sequence was reset because GUID was empty (meaning it the newly created element in a visual list):</color> " + _guid);
        ...
        static void InsertSequentiableTween(ref Sequence s, DOVisualSequence sequence, DOVisualSequenced sequenced, bool isGlobal, float timeMultiplier)
        {
            ...
            if (plug == null || sequenced.plugDataIndex >= plug.pluginDatas.Length) return;
            if (DOVisualSequenceSettings.I.debugLogs) {
                string plugStr = plug.pluginDatas[sequenced.plugDataIndex].label;
                if (isGlobal) {
                    DOLog.Normal(
            ...
            s.Insert(sequenced.startTime * timeMultiplier, plug.CreateTween(sequenced, timeMultiplier));
        }
        ...
        static void InsertAction(ref Sequence s, DOVisualSequence sequence, DOVisualSequenced sequenced, float timeMultiplier)
        {
            ...
            if (plug == null || sequenced.plugDataIndex >= plug.pluginDatas.Length) return;
            PlugDataAction plugDataAction = plug.pluginDatas[sequenced.plugDataIndex];
