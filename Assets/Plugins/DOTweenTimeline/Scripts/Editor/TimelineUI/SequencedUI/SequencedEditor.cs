...
namespace DG.Tweening.TimelineEditor.SequencedUI
{
    internal class SequencedEditor : ABSTimelineElement
    {
        ...
        bool _allPluginsSet, _allPluginsOfSameType, _allPluginsOfSameTypeAndIndex;
        int _totSequenceds;
        ...
        void Init(bool forceRefresh = false)
        {
            ...
            if (_isEvent) {
                // Stop here if Event
                _allTargetsSet = _allTargetsTheSame = _allPluginsSet = _allPluginsOfSameType = _allPluginsOfSameTypeAndIndex = false;
                return;
            }
            //
            _allTargetsSet = _allTargetsTheSame = _allPluginsSet = _allPluginsOfSameType = _allPluginsOfSameTypeAndIndex = true;
            ...
            for (int i = 0; i < _totSequenceds; ++i) {
                ...
                if (_isAction) {
                    if (sequenced == null) {
                        ...
                        if (!isValidPlugin) {
                            plug = null;
                            _allPluginsSet = _allPluginsOfSameType = _allPluginsOfSameTypeAndIndex = false;
                        }
                        int plugIndex = sequenced.plugDataIndex;
                        if (i == 0) {
                            _isGlobal = !isValidPlugin || plug != null && !plug.pluginDatas[_mainSequenced.plugDataIndex].wantsTarget;
                            defPlugId = sequenced.plugId;
                            defPlugIndex = plugIndex;
                        } else {
                            if (_allPluginsOfSameType) {
                                if (sequenced.plugId != defPlugId) _allPluginsOfSameType = _allPluginsOfSameTypeAndIndex = false;
                                else if (plugIndex != defPlugIndex) _allPluginsOfSameTypeAndIndex = false;
                            }
                        }
                    ...
                    if (sequenced == null || !_isGlobal && sequenced.target == null) {
                        ...
                        if (_isGlobal) {
                            ...
                            if (i == 0) {
                                ...
                                if (_allPluginsOfSameType) {
                                    if (plug.targetType != defPlug.targetType) _allPluginsOfSameType = _allPluginsOfSameTypeAndIndex = false;
                                    else if (plugIndex != defPlugIndex) _allPluginsOfSameTypeAndIndex = false;
                                }
                            }
                            _tweenPlugins.Add(plug);
...
//                      + ", _allPluginsOfSameTypeAndIndex: " + LogBool(_allPluginsOfSameTypeAndIndex));
        }
        ...
        public override void Draw(Rect drawArea)
        {
            ...
            if (_isAction) _isGlobal = _actionPlugins.Count > 0 && _mainActionPlugin != null && !_mainActionPlugin.pluginDatas[_mainSequenced.plugDataIndex].wantsTarget;
            ...
            using (new DeGUI.LabelFieldWidthScope(_minLabelWidth)) {
                ...
                if (!_isEvent && !_isInterval && !_isGlobal && (!_isAction || _allPluginsOfSameTypeAndIndex)) _scrollView.IncreaseContentHeightBy(DrawTarget());
                if (!_isEvent && !_isInterval && (!_allTargetsSet || !_allPluginsSet)) {
                    ...
                    if (_isTweener) {
                        ...
                        if (_allPluginsOfSameTypeAndIndex) {
                            Divider();
        ...
        float DrawHeaderAndTweenType()
        {
            ...
            if (!_isInterval) {
                ...
                if (_isAction) {
                    gcPlugin = _allPluginsOfSameTypeAndIndex
                        ? _mainActionPlugin.Editor_GetSequencedHeaderLabelGUIContent(_mainSequenced.plugDataIndex, false)
                        : new GUIContent("–");
                    pluginFullWidth = DOEGUI.Styles.timeline.sBtTargetAndPlugType.CalcSize(gcPlugin).x;
                    drawSelector = true;
                } else if (!_isEvent && (_isGlobal || _allPluginsSet)) {
                    gcPlugin = _allPluginsOfSameTypeAndIndex
                        ? _mainTweenPlugin.Editor_GetShortTypeAndAnimationNameGUIContent(_mainSequenced.plugDataIndex)
        ...
        float DrawTarget()
        {
            ...
            if (_isAction) {
                PlugDataAction plugData = _allPluginsOfSameTypeAndIndex ? _mainActionPlugin.pluginDatas[_mainSequenced.plugDataIndex] : null;
                ...
                for (int i = 0; i < _totSequenceds; ++i) {
                    ...
                    if (!_isAction) {
                        if (sequenced.target == null) {
                            ...
                            if (_tweenPlugins[i] == null || _tweenPlugins[i] != prevPlugin) sequenced.plugDataIndex = 0;
                        }
                    }
                }
            }
        ...
        float DrawEase()
        {
            ...
            if (_allPluginsOfSameTypeAndIndex) {
                switch (_mainTweenPlugin.pluginDatas[_mainSequenced.plugDataIndex].propertyType) {
        ...
        float DrawToAndFrom()
        {
            if (!_allPluginsOfSameTypeAndIndex) return 0;

            Rect fullR = area.SetHeight(_lineHeight + _Padding * 2); // Increased below if necessary
            Rect contentR = fullR.Contract(_Padding);
            Rect innerContentR = contentR;

            PluginTweenType tweenType = _mainTweenPlugin.pluginDatas[_mainSequenced.plugDataIndex].tweenType;
            bool canHaveFrom = CanHaveFrom();
            // From
            if (canHaveFrom) {
                // Flip options
        ...
        float DrawActionOptions()
        {
            ...
            Rect innerContentR = contentR;
            Rect prefixR = innerContentR.SetWidth(_minLabelWidth);
            PlugDataAction plugData = _mainActionPlugin.pluginDatas[_mainSequenced.plugDataIndex];
        ...
        float DrawExtraOptions()
        {
            ...
            ITweenPluginData data = _mainTweenPlugin.pluginDatas[_mainSequenced.plugDataIndex];
            ...
            case PluginTweenType.StringOption:
                ShiftExtraOptionsRectsBy(ref fullR, ref contentR, ref innerContentR, ref prefixR, _lineHeight + _Padding);
                DeGUI.MultiTextField(contentR, new GUIContent(data.stringOptionLabel), "stringOption0", _sequenceds);
                break;
            case PluginTweenType.IntOption:
                ShiftExtraOptionsRectsBy(ref fullR, ref contentR, ref innerContentR, ref prefixR, _lineHeight + _Padding);
                contentR = contentR.SetWidth(_minLabelWidth + 64);
                DeGUI.MultiIntField(contentR, new GUIContent(data.intOptionLabel), "intOption1", _sequenceds);
                break;
            }
        ...
        Rect OptionalAxisConstraint(Rect contentR, DOVisualSequenced.ToFromType toFromType)
        {
            ...
            if (_mainTweenPlugin.pluginDatas[_mainSequenced.plugDataIndex].tweenType != PluginTweenType.SelfDetermined) return contentR;
            if (_mainTweenPlugin.pluginDatas[_mainSequenced.plugDataIndex].propertyType == DOVisualSequenced.PropertyType.Quaternion) return contentR;
            const int width = 20;
        ...
        {
            DOVisualSequenced.PropertyType propertyType = _mainTweenPlugin.pluginDatas[_mainSequenced.plugDataIndex].propertyType;
        ...
        void SwitchToFrom(DOVisualSequenced sequenced, DOVisualTweenPlugin plugin)
        {
            ...
            switch (plugin.pluginDatas[sequenced.plugDataIndex].propertyType) {
            case DOVisualSequenced.PropertyType.Float:
        ...
        void CM_PlugType()
        {
            ...
            SortedPlugData[] sortedPlugDatas = new SortedPlugData[plugin.pluginDatas.Length];
            for (int i = 0; i < plugin.pluginDatas.Length; ++i) {
                    ...
                    false, () => {
                        using (new DOScope.UndoableSerialization()) {
                            for (int j = 0; j < _totSequenceds; ++j) {
                                ...
                                ConditionalResetSequenced(sequenced, plugin, prevPlugin, prevPlugDataIndex);
                            }
                        }
                        Init(true);
                        DOVisualSequenceTimeline.Dispatch_OnSequenceChanged(sequence);
...
//            allPluginsIds.Sort(StringComparer.OrdinalIgnoreCase);
            ...
            for (int i = 0; i < plugins.Count; ++i) {
                ...
                SortedPlugData[] sortedPlugDatas = new SortedPlugData[plugin.pluginDatas.Length];
                for (int j = 0; j < plugin.pluginDatas.Length; ++j) {
                        ...
                        false, () => {
                            using (new DOScope.UndoableSerialization()) {
                                for (int k = 0; k < _totSequenceds; ++k) {
                                    ...
                                    sequenced.plugDataIndex = sortedPData.originalIndex;
                                    Init(true);
                                    ConditionalResetSequenced(sequenced, plugin, prevPlugin, prevPlugDataIndex, true);
                                }
                            }
                        }
                    );
...
//                Array.Sort(sortedPlugDatas, (a, b) => string.Compare(a.data.label, b.data.label, StringComparison.OrdinalIgnoreCase));
        ...
        void CM_CopyValuesFromHierarchyTarget(bool isFrom)
        {
            ...
            PlugDataTween plugData = _mainTweenPlugin.pluginDatas[_mainSequenced.plugDataIndex] as PlugDataTween;
        ...
        bool CanHaveEase()
        {
            switch (_mainTweenPlugin.pluginDatas[_mainSequenced.plugDataIndex].tweenType) {
            case PluginTweenType.Punch:
            case PluginTweenType.Shake:
                return false;
            default: return true;
        ...
        bool CanCopyValuesFromSelectedTarget()
        {
            ...
            if (pluginTargetType == typeof(RectTransform)) {
                return !_mainTweenPlugin.pluginDatas[_mainSequenced.plugDataIndex].label.Contains("AnchoredPosition")
                       || Selection.activeTransform.GetComponent<RectTransform>() != null;
            } else if (pluginTargetType == typeof(Transform)) return true;
            return false;
        }

        void ConditionalResetSequenced(DOVisualSequenced sequenced, DOVisualTweenPlugin plugin, DOVisualTweenPlugin prevPlugin, int prevPlugDataIndex, bool forceReset = false)
        {
            ...
            PluginTweenType tweenType = plugin.pluginDatas[sequenced.plugDataIndex].tweenType;
            if (forceReset || prevPlugin != null && tweenType != prevPlugin.pluginDatas[prevPlugDataIndex].tweenType) {
                switch (tweenType) {
                case PluginTweenType.Punch:
                case PluginTweenType.Shake:
        ...
        void ConditionalResetSequenced(DOVisualSequenced sequenced, DOVisualActionPlugin plugin, DOVisualActionPlugin prevPlugin, int prevPlugDataIndex)
        {
            PlugDataAction plugData = plugin.pluginDatas[sequenced.plugDataIndex];
            bool resetTarget = prevPlugin != null &&
                               (prevPlugin.pluginDatas[prevPlugDataIndex].wantsTarget != plugData.wantsTarget
                               || prevPlugin.pluginDatas[prevPlugDataIndex].targetType != plugData.targetType);
