            ...
            for (int i = 0; i < len; ++i) {
                ...
                if (isMainPass) {
                    ...
                    if (isAction) {
                        ...
                        hasPlugin = actionPlugin != null && sequenced.plugDataIndex < actionPlugin.pluginDatas.Length;
                        isGlobal = !hasPlugin || actionPlugin.pluginDatas[sequenced.plugDataIndex].wantsTarget == false;
                    } else {
                    ...
                    bool hasError = !isEvent && !isInterval && (!isGlobal && !hasTarget || !hasPlugin);
                    if (hasError) {
                        // Missing target
                    ...
                    if (isInterval) {
                        Rect labelR = hasError ? contentR : contentR.Shift(-icoR.width + 2, 0, icoR.width - 4, 0);
                        ...
                        GUI.Label(labelR,
                            !hasPlugin
                                ...
                                : string.Format("<b>{0}</b>", isAction
                                    ? actionPlugin.Editor_GetSequencedHeaderLabelGUIContent(sequenced.plugDataIndex, true).text
                                    : tweenPlugin.Editor_GetAnimationNameGUIContent(sequenced.plugDataIndex, true).text
                                ),
                        ...
                        GUI.Label(contentR.Shift(0, 0, -2, 0),
                            !hasPlugin
                                ? string.Format(
        ...
        void AddSequenced(DOVisualSequenced sequenced, DOVisualSequence.VisualLayer layer, bool isCopy = false)
        {
            using (new DOScope.UndoableSerialization()) {
                ...
                if (!isCopy) {
                    ...
                    sequenced.duration = sequenced.type == DOVisualSequenced.Type.Action || sequenced.type == DOVisualSequenced.Type.Event
                        ? 0
